...
A voter is not a strength. Strengths are common; voters are the one
or two traits held at such an idiosyncratic extreme that they decide
...
1. **Extremity, earned.** Not competence — excess. "Magic is just
   someone spending more time on something than anyone else might
   reasonably expect." The evidence is a history: years of obsession,
...
A voter does not need to be objectively superior; it needs to be
*yours*. A "refuted" chess opening beats prepared opponents at your
level precisely because nobody prepares for it — competitors who
...
### Generate willingly; gate mercilessly

Harvest candidates liberally — from the self-portrait file, the
...
obsession cost you that a reasonable person wouldn't have paid?" A
candidate the user is stuck defending gets your best steelman first,
then the honest kill. Two bookkeeping notes: when the evidence
pressed out of a dying candidate really belongs to another (the
"craftsmanship" facts were all about the tooling stack), cut the
word and transfer the evidence — record the transfer in both places;
and a candidate may be *cut-merged* into another (a drive that is
just the fuel of an existing voter) — cut list plus evidence, no
independent entry.
...
- **Walk the weaknesses** (for each settled voter): from the
  ...
  the ideal customer read as *service to the voter*? Record them
  under the voter. A voter that overpowers nothing gets one more
...
> complete; ranking / weakness walk / closing press / finalization
> remaining."> If you are resuming, continue there. (Removed at
> finalization; episode evidence worth keeping folds into the
> surviving entries first.)

<Two or three lines: whose voters, which input files ([M]/[S]/[K])
this distills, what strategy question it serves.>
...
- <Trait> — <one line: real, but common among peers / no overpowered
  cost named / decides nothing downstream>.    [S1]
...
- <Candidate> — <one line of reasoning>.

## Next steps

<Two or three sentences of prose: audit strategy against the voters
...
frozen number with the user's confirmation while in progress.
